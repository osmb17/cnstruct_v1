...
with inp_col:
    ...
    # ======================================================================
    # PRESETS — save / load named input sets (per structure type)
    # ======================================================================
    _presets     = hist.list_presets(template_name)
    ...
    with st.expander("📋 Presets", expanded=False):
        if _preset_names:
            ...
            _sel_name = _pc1.selectbox(
                "Saved presets", _preset_names,
                key=f"_preset_sel_{_tname}", label_visibility="collapsed",
            )
            _sel_idx = _preset_names.index(_sel_name) if _sel_name in _preset_names else 0
            _sel_id  = _preset_ids[_sel_idx]

            if _pc2.button("Load", key=f"_preset_load_{_tname}", use_container_width=True):
                _pparams = hist.load_preset_params(_sel_id)
                for _pf in template.inputs:
                    _pv = _pparams.get(_pf.name)
                    if _pv is None:
                        continue
                    _pwkey = f"primary_{_tname}__{_pf.name}"
                    # ft fields stored as floats → convert back to ft-in string
                    if _pf.name.endswith("_ft") and isinstance(_pv, (int, float)):
                        st.session_state[_pwkey] = _format_ft_in(float(_pv))
                    else:
                        st.session_state[_pwkey] = str(_pv)
                st.rerun()

            if _pc3.button("🗑", key=f"_preset_del_{_tname}", use_container_width=True,
                           help="Delete this preset"):
                hist.delete_preset(_sel_id)
                st.rerun()
        else:
            st.caption("No saved presets for this structure type yet.")

        # Save row
        _psn_col, _psb_col = st.columns([3, 1])
        _preset_save_name = _psn_col.text_input(
            "Preset name", key=f"_preset_name_{_tname}",
            placeholder="e.g. Standard 8ft D89A",
            label_visibility="collapsed",
        )
        if _psb_col.button("Save", key=f"_preset_save_{_tname}", use_container_width=True):
            _pname = (_preset_save_name or "").strip()
            if _pname:
                _snap: dict = {}
                for _pf in template.inputs:
                    _pwkey  = f"primary_{_tname}__{_pf.name}"
                    _pawkey = f"adv_{_tname}__{_pf.name}"
                    if _pwkey in st.session_state:
                        _snap[_pf.name] = st.session_state[_pwkey]
                    elif _pawkey in st.session_state:
                        _snap[_pf.name] = st.session_state[_pawkey]
                    else:
                        _snap[_pf.name] = _pf.default
                hist.save_preset(template_name, _pname, _snap)
                st.success(f"✓ Saved preset \"{_pname}\"")
                st.rerun()
            else:

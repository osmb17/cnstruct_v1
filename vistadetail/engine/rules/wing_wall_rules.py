...
def rule_wing_horiz(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    """
    usable_h = (p.hw_height_ft * 12) - (2 * p.cover_in)
    qty_per_face = math.floor(usable_h / p.horiz_spacing_in) + 1
    ...
    log.result("WH1", f"{p.horiz_bar_size} × {qty_total} @ {fmt_inches(bar_len_in)} [EF]")
...
def rule_wing_vert(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    """
    usable_len = (p.wing_length_ft * 12) - (2 * p.cover_in)
    qty_per_face = math.floor(usable_len / p.vert_spacing_in) + 1
    qty_total = qty_per_face * 2
    ...
    log.result("WV1", f"{p.vert_bar_size} × {qty_total} @ {fmt_inches(bar_len_in)} [EF]")
...
def rule_wing_corner(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    log.result("WC1", f"{p.horiz_bar_size} × {qty}  L {fmt_inches(leg)} × {fmt_inches(leg)}")

    return [BarRow(
        mark="WC1", size=p.horiz_bar_size, qty=qty,
...
def rule_validate_wing(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    if p.cover_in < 2.0:
        log.warn(f"Cover {p.cover_in} in < 2 in min for exposed wing wall")
    return []

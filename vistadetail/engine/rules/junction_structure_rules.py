...
def rule_junction_long_wall_horiz(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    bar_len_in     = outside_len_in - (2 * 2.0)
    usable_h_in    = (p.inside_depth_ft * 12) - (2 * 2.0)
    ...
    log.result("JW1", f"#5 × {qty} @ {fmt_inches(bar_len_in)}", source="JunctionRules")

    return [BarRow(
        mark="JW1", size="#5", qty=qty, length_in=bar_len_in,
...
def rule_junction_long_wall_vert(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    """
    usable_len_in  = (p.inside_length_ft * 12) - (2 * 2.0)
    ...
    log.result("JW2", f"#5 × {qty} @ {fmt_inches(bar_len_in)}", source="JunctionRules")

    return [BarRow(
        mark="JW2", size="#5", qty=qty, length_in=bar_len_in,
...
def rule_junction_short_wall_horiz(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    """
    bar_len_in   = (p.inside_width_ft * 12) - (2 * 2.0)
    usable_h_in  = (p.inside_depth_ft * 12) - (2 * 2.0)
    qty_per_face = math.floor(usable_h_in / 12.0) + 1
    qty          = qty_per_face * 2 * 2   # 2 faces × 2 short walls

    log.step(
        f"Short wall horiz: bar length = {p.inside_width_ft * 12:.0f} − 2×{2.0} "
        f"= {bar_len_in:.1f} in = {fmt_inches(bar_len_in)}",
        source="JunctionRules",
    )
    log.step(f"Qty = {qty_per_face} × 2 faces × 2 walls = {qty}", source="JunctionRules")
    log.result("SW1", f"#5 × {qty} @ {fmt_inches(bar_len_in)}", source="JunctionRules")

    return [BarRow(
        mark="SW1", size="#5", qty=qty, length_in=bar_len_in,
...
def rule_junction_short_wall_vert(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    """
    usable_w_in  = (p.inside_width_ft * 12) - (2 * 2.0)
    qty_per_face = math.floor(usable_w_in / 12.0) + 1
    qty          = qty_per_face * 2 * 2   # 2 faces × 2 short walls
    bot_hook_in  = hook_add("std_90", "#5")
    bar_len_in   = (p.inside_depth_ft * 12) + bot_hook_in + 6.0
    ...
    log.result("SW2", f"#5 × {qty} @ {fmt_inches(bar_len_in)}", source="JunctionRules")

    return [BarRow(
        mark="SW2", size="#5", qty=qty, length_in=bar_len_in,
        shape="L", leg_a_in=p.inside_depth_ft * 12 + 6.0, leg_b_in=bot_hook_in,
...
def rule_junction_floor_long(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    outside_len_in = (p.inside_length_ft * 12) + (2 * p.wall_thick_in)
    bar_len_in     = outside_len_in - (2 * 2.0)
    qty            = math.floor((p.inside_width_ft * 12) / 12.0) + 1
    ...
    log.step(f"Qty = ⌊{p.inside_width_ft * 12:.0f}/{12.0}⌋ + 1 = {qty}", source="JunctionRules")
    log.result("JF1", f"#5 × {qty} @ {fmt_inches(bar_len_in)}", source="JunctionRules")

    return [BarRow(
        mark="JF1", size="#5", qty=qty, length_in=bar_len_in,
...
def rule_junction_floor_short(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    """
    bar_len_in = (p.inside_width_ft * 12) - (2 * 2.0)
    qty        = math.floor((p.inside_length_ft * 12) / 12.0) + 1

    log.step(
        f"Floor short: length = {p.inside_width_ft * 12:.0f} − 2×{2.0} "
        f"= {bar_len_in:.1f} in = {fmt_inches(bar_len_in)}",
        source="JunctionRules",
    )
    log.step(f"Qty = ⌊{p.inside_length_ft * 12:.0f}/{12.0}⌋ + 1 = {qty}", source="JunctionRules")
    log.result("JF2", f"#5 × {qty} @ {fmt_inches(bar_len_in)}", source="JunctionRules")

    return [BarRow(
        mark="JF2", size="#5", qty=qty, length_in=bar_len_in,
...
def rule_validate_junction(p: Params, log: ReasoningLogger) -> list[BarRow]:
    ...
    if 2.0 < 2.0:
        log.warn(f"Cover {2.0} in < 2 in minimum for junction structure")
    wall_max_sp = min(3 * p.wall_thick_in, 18.0)
    if 12.0 > wall_max_sp:
        log.warn(
            f"Horiz spacing {12.0} in > ACI max {wall_max_sp} in "

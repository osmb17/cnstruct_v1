"""
Caltrans D72 CIP Drainage Inlet Type G6.

Same-slope gutter type (no depression) -- #4 all around. Standard W = 2'-11¾".
Grate flush with gutter grade, no flowline depression.
"""
...
class G6InletTemplate(BaseTemplate):
    def __init__(self):
        super().__init__()
        self.name = "G6 Inlet"
        self.version = "1.0"
        self.description = (
            "Caltrans D72B -- CIP Drainage Inlet Type G6. "
            "Same-slope gutter, no depression, #4 all around, std W=2'-11¾\"."
        )
        self.inputs = [
